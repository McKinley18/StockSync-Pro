...
if (Platform.OS === 'web') {
  ...
  databaseModule = {
    CATEGORIES,
    UNITS: ['items', 'lbs', 'oz', 'kg', 'g', 'ml', 'liters', 'cartons', 'bags', 'boxes'],
    initDatabase: () => {
      db.execSync(`
        ...
        CREATE TABLE IF NOT EXISTS saved_recipes (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, instructions TEXT);
    ...
    addRecipe: (recipe: any, ingredients: any[]) => {
      const recipeRes = db.runSync('INSERT INTO saved_recipes (name, instructions) VALUES (?, ?)', [recipe.name, recipe.instructions]);
...
export interface SavedRecipe { id?: number; name: string; instructions: string; }
